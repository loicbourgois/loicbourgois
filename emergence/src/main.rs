...
// TODO: for each bucket, find max and min
// then print a vertical line from min to max 
// we want to know teh range at each period
fn print_death_chart(history: &[Metric]) {
    if history.is_empty() {
...
fn choose_action(
    ...
    if agent.state["motivation"].v > rng.gen_range(0.0..=1.0) {
        Action::FindFood
    }
...
fn step(
    ...
    apply_passive_updates(agent, community, rules);
    live_or_die(agent);
    agent.age += 1;
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for _turn in 0..TURNS {
        for agent in &mut simulation.agents {
            step(agent, &mut community, &simulation.rules, &mut rng);
        }
