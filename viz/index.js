...
navigator.requestMIDIAccess().then(function(access) {
  Array.from(access.inputs).forEach(input => {
    input[1].onmidimessage = (msg) => {
      const control_id = msg.data[1];
      const control_value = msg.data[2];
      const controller_id = 0;
      const str = `slider_${control_id}`
      const slider = document.querySelector(`#${str}`)
      try {
        slider.value = control_value / 127.0 * slider.max
      } catch (e) {
        console.warn(`slider ${str} does not exist`)
      } finally {

      }
      document.getElementById("controller_id").innerHTML = "" + controller_id;
      document.getElementById("control_id").innerHTML = "" + control_id;
      document.getElementById("control_value").innerHTML = "" + control_value;
    }
  })
