...
const set_center = (context) => {
  center.x = context.canvas.width/2
  center.y = context.canvas.height/2
}
...
const stroke_circle_cc = (context, cc, radius, color, lineWidth) => {
  context.beginPath();
  context.arc(cc.x + center.x, center.y - cc.y , radius, 0, 2 * Math.PI, false);
  context.strokeStyle = color;
  context.lineWidth = lineWidth?lineWidth:2;
  context.stroke();
}
...
const line_cc = (context, cc1, cc2, color, line_width) => {
  context.beginPath();
  context.moveTo(cc1.x + center.x, - cc1.y + center.y);
  context.lineTo(cc2.x + center.x, - cc2.y + center.y);
  context.strokeStyle = color;
  context.lineWidth = line_width?line_width:2;
  context.stroke();
}
